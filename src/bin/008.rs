fn main() {
    let number_of_adjacent_digits = 13;
    let number_series = vec![
    ...
    let mut greatest_product: u128 = 0;

    for p in 0..(number_series.len() - number_of_adjacent_digits) {
        let mut product = 1;
        for n in p..(p + number_of_adjacent_digits) {
            product *= number_series[n];
        }
        if product > greatest_product {
            print!("{}", number_series[p]);
            for n in 1..number_of_adjacent_digits {
